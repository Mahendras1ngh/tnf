...
// ISR: pre-render every service page at build and refresh hourly. This keeps
// the pages fully static (fast + crawlable) with no per-request DB dependency.
...
// Pre-generate all active service pages. Guarded so a build-time DB blip
// falls back to on-demand rendering instead of failing the whole build.
export async function generateStaticParams() {
  try {
    ...
    console.error('generateStaticParams (services) failed:', error);
    return [];
  }
}
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  // Service + breadcrumb structured data — helps Google understand the offering
  // and show breadcrumb rich results in search.
  const serviceJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: service.title,
    ...
    url: `${SITE_URL}/services/${service.slug}`,
    image: service.image || `${SITE_URL}/og-image.png`,
