...
// ISR: pre-render published posts at build, refresh hourly (never force-dynamic).
...
export async function generateStaticParams() {
  try {
    ...
    console.error('generateStaticParams (journal) failed:', error);
    return [];
  }
}
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const articleJsonLd = {
    ...
    headline: post.title,
    description: post.excerpt || post.title,
    image: post.coverImage ? [post.coverImage] : [`${SITE_URL}/og-image.png`],
    datePublished: post.publishedAt?.toISOString(),
    dateModified: post.updatedAt?.toISOString(),
    author: {
      '@type': post.author?.name ? 'Person' : 'Organization',
      name: post.author?.name || 'The Next Frame',
    },
    publisher: {
      '@type': 'Organization',
      name: 'The Next Frame',
      logo: {
        '@type': 'ImageObject',
        url: `${SITE_URL}/assets/brand/circle-logo.png`,
      },
    },
    mainEntityOfPage: {
      '@type': 'WebPage',
